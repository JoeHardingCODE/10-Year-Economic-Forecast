...
#Pulling the data from IMf.data
...
pull_dots_q <- function(reporter, partner) {
  ...
  )   #Creating a matrix for the in program storage of the data
  cols <- names(df)
...
#summing trade with other countries, then summing trade within the two countries--> then 
# representing this as an index value (2018 average = index 100)
...
events <- tibble(
  ...
  label = c("Trade tensions","COVID19 Hits US","Ukraine War Begins")
)
